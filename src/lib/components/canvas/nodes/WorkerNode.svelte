...
<div
	class="min-w-[140px] rounded shadow-sm cursor-move overflow-hidden transition-all bg-node-fill {selected ? 'ring-2 ring-selected shadow-md' : ''}"
	style="border: 2px dashed var(--color-node-stroke);"
	onclick={edit.handleClick}
	ondblclick={edit.startEdit}
>
	<div class="px-3 py-1 bg-node-header">
		<span class="text-node-kind text-xs font-semibold uppercase tracking-wide">worker</span>
	</div>
	<div class="px-3 py-2">
		{#if edit.editing}
			<input
